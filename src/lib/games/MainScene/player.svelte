...
// @ts-nocheck
    ...
    useTask((delta)=>{
        if(gameState.globalRoom){
            ...
            if(gameState.camera){
                ...
                if(pinfo.id == gameState.globalRoom.sessionId){
            ...
            if(gameState.globalRoom.sessionId == pinfo.id){
    ...
    function handleMouseDown(event) {
        ...
        if (event.button !== 0 || !gameState.camera || !gameState.globalRoom || 
            gameState.globalRoom.sessionId != pinfo.id
        ) return;
    ...
    function handleMouseUp(){
        if(gameState.globalRoom && gameState.globalRoom.sessionId == pinfo.id && onAttack){
            onAttack = false
        }
    }
</script>
...
>
    ...
    }} mass={1} type="dynamic" userData={{type:"player",id:pinfo.id}} >
        <AutoColliders shape={"cuboid"}>
            <T.Mesh castShadow position={[0,0,0]} >
                <T.BoxGeometry args={[0.5,0.5,0.5]} />
                <T.MeshStandardMaterial color={pinfo.color} />
                <Outlines color="black" />
            </T.Mesh>
        </AutoColliders>
    
        <T.Mesh bind:ref={headRef} castShadow position={[0,0,0.5]}>
        ...
        <T.Group scale={[scaleFactor,scaleFactor,scaleFactor]}  position={[0,1.5,0]}>
            <Billboard  >
                ...
                flexDirection={"column"} backgroundOpacity={0} borderRadius={5} backgroundColor="transparent">
                    <!-- Pseudo du joueur -->
                    <Text  text={pinfo.id} fontWeight={"bold"} fontSize={20} color="white" />
                    ...
                    <Container  marginTop={5}  padding={3} borderRadius={100} backgroundColor={"#f7fff6"}>
                        ...
                        <Container
                            ...
                            height={20}
                            backgroundColor={pinfo.id == gameState.globalRoom.sessionId?"#20bf55":"#8b5fbf"}
                            alignX="left"
                            borderRadius={100}
                            />
                        </Container>
                    </Container>
                    <Text text={`${Math.round($player_pv)}/${player_pv_max}`} fontWeight={"extra-bold"} fontSize={15} color="white" />
                </Root>
            </Billboard>
        </T.Group>
    </RigidBody>
